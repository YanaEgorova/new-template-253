...
function cartAdd(e) {
  ...
  let total = parseFloat(
    parent.querySelector('.js_prod__price').textContent.substr(1)
  );
  console.log(total);
  let okay = true;
    ...
    if (localStorage(id, amount.textContent) != false) {
      if (cartSpan) {
        cartSpan.textContent =
          Number(amount.textContent) + Number(cartSpan.textContent);
      }
      setAmountToCartSpan();
      const successMessageSpan = document.querySelector(
        '.js_success-product-name'
      );
...
function calculateTotalNutra(amount, product) {
  priceSpan.textContent = `${amount}x for $${product.nutraRange[
    amount - 1
  ].toFixed(2)}`;
}
